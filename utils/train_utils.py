...
def train_defect(defected, clear, defect_gt, model, phase, device):
    print("                             ")
    print("                             TRAINING DR")
    print("                             ")
...
# set the loss function:
        ...
        print("loss",loss_mse,"\n")
        # loss_mse_scale = compute_mse(scale_cal_argmax, groundTruth_scale)
        # print("loss rot ==", loss_rot)
        # print("loss scale ==", loss_scale)
        # g=make_dot(loss_mse)
        # g.view()
        # total_loss = loss_mse
        total_loss = loss_mse+0.001*loss_angle
        return  total_loss, synthesis_unet[:,0,:].unsqueeze(1), synthesis_unet[:,1,:].unsqueeze(1), clear_transformed, defection_map, defect_gt_blur, rot_cal
